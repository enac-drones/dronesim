...
class RPYTAviary(BaseAviary):
    ...
    def _actionSpace(self):
        ...
        # act_upper_bound = np.array([self.THRUST2WEIGHT_RATIO,  1,  1,  1])
        return spaces.Dict({str(i): spaces.Box(low= np.array([0.,              -1, -1, -1]),
                                               high=np.array([self.drones[i].THRUST2WEIGHT_RATIO,  1,  1,  1]),
                                               dtype=np.float32
    ...
    def _observationSpace(self):
        ...
        obs_upper_bound = np.array([np.inf,  np.inf,  np.inf, 1.,  1.,  1.,  1.,  np.pi,  np.pi,  np.pi,  np.inf,  np.inf,  np.inf,  np.inf,  np.inf,  np.inf,  np.inf, np.inf,  np.inf,   np.inf])
    ...
    def _preprocessAction(self,
        ...
        for k, v in action.items():
            ...
            rpms = self.ctrl[int(k)]._INDIRateControl(
            control_timestep=self.AGGR_PHY_STEPS * self.TIMESTEP,
            thrust=v[3],
            cur_quat=state[3:7],
            cur_ang_vel=state[13:16],
            target_rpy_rates=v[:3])

            preprocessed_action[k] = rpms
