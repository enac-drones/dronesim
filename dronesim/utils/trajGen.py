...
from dronesim.utils.trajutils import DesiredState, Hessian, polyder

"""
source: https://github.com/Bharath2/Quadrotor-Simulation/blob/main/TrajGen/trajGen.py
...
class trajGenerator:
    ...
    def optimize(self):
        ...
        T = optimize.minimize(
            ...
            Tmin,
            method="COBYLA",
            constraints=({"type": "ineq", "fun": lambda T: T - Tmin}),
        )["x"]
    ...
    def get_constraints(self, T):
        ...
        for i in range(n - 1):
            A[2 * n + 4 * i : 2 * n + 4 * (i + 1), o * i : o * (i + 1)] = -polyder(
                T[i], "all"
            )
            A[2 * n + 4 * i : 2 * n + 4 * (i + 1), o * (i + 1) : o * (i + 2)] = polyder(
                0, "all"
            )
        ...
        # free variables
        for i in range(1, n):
            A[6 * n + 4 * i : 6 * n + 4 * (i + 1), o * i : o * (i + 1)] = polyder(
                0, "all"
            )
