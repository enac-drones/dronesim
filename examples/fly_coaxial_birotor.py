...
# from dronesim.envs.VisionAviary import VisionAviary
...
if __name__ == "__main__":
    ...
    parser.add_argument('--drone',              default=['coaxial_birotor'],  type=list,    help='Drone model (default: CF2X)', metavar='', choices=[DroneModel])
    ...
    INIT_RPYS = np.array([[0, 0,  i * (np.pi/20.)/ARGS.num_drones] for i in range(ARGS.num_drones)])
    ...
    ## Hover ###
    INIT_XYZS = np.array([[0., 0., 0.6]])
    INIT_RPYS = np.array([[0., 0.*3.14/180., 0.*3.14/180.]])
    INIT_VELS = np.array([[0., 0., 0.]])
    ...
    for i in range(NUM_WP):
        TARGET_POS[i, :] = R*np.cos((i/NUM_WP)*(4*np.pi)+np.pi/2)+INIT_XYZS[0, 0], R*np.sin((i/NUM_WP)*(4*np.pi)+np.pi/2)-R+INIT_XYZS[0, 1], 0
    wp_counters = np.array([int((i*NUM_WP/6)%NUM_WP) for i in range(ARGS.num_drones)])

    TARGET_RPYS =np.zeros((NUM_WP,3))
    for i in range(NUM_WP):
        TARGET_RPYS[i, :] = [0., 0., 0.] #0.4+(i*1./200)]
    ...
    #     ctrl = [INDIControl(drone_model=ARGS.drone) for i in range(ARGS.num_drones)]
    ...
    action = {str(i): np.array([0.25,0.25,0.0,0.0]) for i in range(ARGS.num_drones)}
