...
if __name__ == "__main__":
    ...
    INIT_XYZS = np.array([[R*np.cos((i/6)*2*np.pi+np.pi/2), R*np.sin((i/6)*2*np.pi+np.pi/2), H+i*H_STEP] for i in range(ARGS.num_drones)])
    INIT_RPYS = np.array([[0., 0., 0.] for i in range(ARGS.num_drones)])

    #### Create the environment
    env = VelocityAviary(drone_model=ARGS.num_drones*ARGS.drone,
